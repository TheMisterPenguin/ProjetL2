...
list * init_liste(void *(*fonction_ajout)(void *), void (*fonction_suppresion)(void *),
    ...
    _new->flag = (t_element *) malloc(sizeof(t_element));
    _new->flag->pred = _new->flag;
    _new->flag->succ = _new->flag;
    _new->ec = _new->flag;
    _new->nb_elem = 0;
    _new->ajout = fonction_ajout;
    _new->del = fonction_suppresion;
    _new->aff = fonction_affichage;

    return _new;
}

bool liste_vide(const list * const to_verify) {return (to_verify->flag->pred == to_verify->flag);}

bool hors_liste(const list * const to_verify) {return to_verify->ec == to_verify->flag;}
...
bool selectionner_element(list * liste, void * element, bool (*f_egalite)(void *, void *)){
