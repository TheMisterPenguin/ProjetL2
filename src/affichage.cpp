...
t_aff * creer_texture(const char* nom_fichier, const int taille_t_x, const int taille_t_y, const int x, const int y, const float multiplicateur_taille){
    ...
    texture = (t_aff *) malloc(sizeof(t_aff));
    ...
    if(taille_t_x > -1 && taille_t_y > -1){
        texture->frame_anim = (SDL_Rect *) malloc(sizeof(SDL_Rect));
    ...
    texture->aff_fenetre = (SDL_Rect *) malloc(sizeof(SDL_Rect));
...
t_l_aff* init_textures_joueur(joueur_t *j, int num_j){
    t_l_aff* textures_joueur = (t_l_aff *) malloc(sizeof(t_l_aff));
    textures_joueur->nb_valeurs = NB_SPRITE_JOUEUR;
    textures_joueur->liste = (t_aff **) malloc(sizeof(t_aff)*NB_SPRITE_JOUEUR);
...
t_aff *next_frame_joueur(joueur_t *j)
{
    ...
    if(statut->action == J_BLESSE){
        ...
        /* action terminé */
            ...
            statut->action = RIEN;
            statut->orient_dep = (t_direction_1) current_frame_y(statut->texture_prec);
        ...
        else if (statut->action == ATTAQUE)
        {
            ...
            { 
                ...
                if(statut->orient_dep != EST_1)
                    statut->orient_att = (t_direction_2) ((statut->orient_att + 7) % 8);
                /* mouvement de l'épée dans le sens horraire pour la direction EST */
                else
                    statut->orient_att = (t_direction_2) ((statut->orient_att + 1) % 8);
...
void afficher_monstres(list * liste_monstre, joueur_t * joueur){
    ...
    while(!hors_liste(liste_monstre)){
        monstre = (monstre_t *) valeur_elt(liste_monstre);
...
void afficher_coffres(list * liste_coffre){
    ...
    while(!hors_liste(liste_coffre)){
        coffre = (coffre_t*) valeur_elt(liste_coffre);
...
void afficher_sorts(list * liste_sorts, joueur_t * joueur){
    ...
    while(!hors_liste(liste_sorts)){
        sort = (sort_t*) valeur_elt(liste_sorts);
...
err_t afficher_buffer(list *buffer, SDL_Renderer *rendu){
    ...
    while(!hors_liste(buffer)){
        ...
        if(afficher_texture((t_aff*) valeur_elt(buffer), rendu) < 0){ /* On affiche la texture actuelle à l'écran */
...
bool deplacement_x_pers(t_map *m, joueur_t ** joueurs, unsigned short int nb_joueurs, int x, lobjet_t * objets){
    ...
    while(!hors_liste(m->liste_zone_tp)){
        zone_tp *z = (zone_tp*) valeur_elt(m->liste_zone_tp);
    ...
    while(!hors_liste(m->liste_collisions)){
        SDL_Rect *element = (SDL_Rect  *) valeur_elt(m->liste_collisions);
...
void afficher_animations(list * animations){
    ...
    while(!hors_liste(animations) && animations->ec->valeur != NULL){ //évite de boucler à l'infini en cas d'erreur
        afficher_texture((t_aff *)animations->ec->valeur, rendu_principal);
