...
void creer_sauvegarde_json(joueur_t *j){
	...
	char *path = (char *)calloc(strlen(save_path) + 15, sizeof(char));
...
joueur_t *charger_sauvegarde_joueur(char *nom_sauv, char * f_src_obj, joueur_t *joueurs[], unsigned short int nb_joueurs){
	...
	byte *trigger_tab = (byte *) calloc(TAILLE_TRIGGER, sizeof(byte));
	...
	joueur_t * j = creer_joueur(
		...
		trigger_tab,
		(t_direction_1) json_object_get_int(orientation),
...
joueur_t *new_joueur(const char* nom, int num_j, char * f_src_obj){
	...
	byte *trig = (byte *) calloc(TAILLE_TRIGGER, sizeof(byte));
...
{
	joueur_t * perso = (joueur_t *) malloc(sizeof(joueur_t));

	if(!perso)
		erreur("Erreur lors de la création du joueur : Plus assez de mémoire !\n", OUT_OF_MEM);

	perso->nom_pers = (char *) malloc(sizeof(char) * (strlen(nom) + 1));
	...
	perso->trigger = (byte *) calloc(TAILLE_TRIGGER, sizeof(byte));
	...
	perso->statut = (statut_t *) malloc(sizeof(statut_t));
...
SDL_bool entite_subit_attaque(SDL_Rect * monstre_hitbox, joueur_t * joueur){
	...
	SDL_Rect * hitbox = (SDL_Rect*) malloc(sizeof(SDL_Rect));
...
bool entite_en_collision(SDL_Rect * entite_1, SDL_Rect * entite_2, t_direction_1 * cote_entite_1, t_direction_1 * cote_entite_2){
	SDL_Rect temp;
	temp.w = entite_1->w;
	temp.h = entite_1->h;
	temp.y = entite_1->y;
...
void environnement_joueurs(list * liste_monstres, list * liste_sorts, list * liste_coffres, joueur_t ** joueurs, int nb_joueur){
	...
	for(i=0; i < nb_joueur; i++){
		...
		while(!hors_liste(liste_monstres)){
			monstre = (monstre_t*) valeur_elt(liste_monstres);
